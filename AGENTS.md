...
- **Validate every event-sourced pubkey or event id** with `isNostrId()` from `@/lib/nostrId` before passing it to `nip19.*Encode`, route params, or filter `authors`/`ids` arrays — the encoders throw on malformed hex and crash the rendering subtree. Validate at the parse layer (so renderers can assume hex is well-formed) or use the `tryNpubEncode`/`tryNeventEncode`/`tryNaddrEncode` wrappers from `@/lib/safeNip19` at the boundary.
